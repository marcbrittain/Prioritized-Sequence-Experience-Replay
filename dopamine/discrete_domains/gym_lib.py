...
LUNARLANDER_MIN_VALS = np.array([-1., -1., -1., -1., -1., -1., -1., -1.])
LUNARLANDER_MAX_VALS = np.array([ 1.,  1.,  1.,  1.,  1.,  1.,  1.,  1. ])
gin.constant('gym_lib.LUNARLANDER_OBSERVATION_SHAPE', (8, 1))
gin.constant('gym_lib.LUNARLANDER_OBSERVATION_DTYPE', tf.float32)
gin.constant('gym_lib.LUNARLANDER_STACK_SIZE', 1)
...
@gin.configurable
def lunarlander_dqn_network(num_actions, network_type, state):
  """Builds the deep network used to compute the agent's Q-values.
  ...
  """
  q_values = _basic_discrete_domain_network(
      LUNARLANDER_MIN_VALS, LUNARLANDER_MAX_VALS, num_actions, state)
  return network_type(q_values)
