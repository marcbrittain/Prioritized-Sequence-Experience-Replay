...
class OutOfGraphReplayBuffer(object):
  ...
  def __init__(self,
    ...
    tf.logging.info('\t decay_scheme: %s', decay_scheme)
    tf.logging.info('\t decay_window: %s', decay_window)
    tf.logging.info('\t decay_gamma: %s', decay_gamma)
    tf.logging.info('\t decay_schedule: %s', decay_schedule)
    tf.logging.info('\t adaptive: %s', adaptive)
    ...
    self._decay_scheme = decay_scheme
    ...
    if self._decay_schedule != 'none':
        frames = self._decay_schedule
        self._decay_gamma_init = self._decay_gamma
        self._decay_gamma_rate     = get_annealing_rate(self._decay_gamma, 0.4, frames)




  def get_per_gamma(self, total_t):
        return max(get_annealed_value(self._decay_gamma_rate,self._decay_gamma_init , total_t), 0.4)
